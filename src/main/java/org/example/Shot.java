...
public class Shot implements UserInput{
    public void shot(String[][] grid1, String[][] grid2, List<Ship> shipListPlayer1,List<Ship> shipListPlayer2) {
        ...
        do {
            ...
            endCondition = player1Shot(grid1, grid2, endCondition, row, column,shipListPlayer2);
    ...
    private static boolean player1Shot(String[][] grid1, String[][] grid2, boolean endCondition, int row, int column, List<Ship> shipListPlayer2) {
        if (grid2[row][column].equals("O") & row < grid1.length & row > 0 & column < grid1.length & column > 0) {

            boolean msgCondition = true;

            resetCoordinates(row, column, 8, shipListPlayer2.get(0).getShiptype().getCoordinates());
            msgCondition = shipFiveSunk(msgCondition, shipListPlayer2.get(0).getShiptype().getCoordinates());

            resetCoordinates(row, column, 7, shipListPlayer2.get(1).getShiptype().getCoordinates());
            msgCondition = shipFourSunk(msgCondition, shipListPlayer2.get(1).getShiptype().getCoordinates());
    ...
    private static boolean player2Shot(String[][] grid1, String[][] grid2, boolean endCondition, int row, int column, List<Ship> shipListPlayer1) {
        if ("O".equals(grid1[row][column]) & row < grid1.length & row > 0 & column < grid1.length & column > 0) {
            grid1[row][column] = "X";
            boolean msgCondition = true;

            resetCoordinates(row, column, 8, shipListPlayer1.get(0).getShiptype().getCoordinates());
            msgCondition = shipFiveSunk(msgCondition, shipListPlayer1.get(0).getShiptype().getCoordinates());

            resetCoordinates(row, column, 7, shipListPlayer1.get(1).getShiptype().getCoordinates());
            msgCondition = shipFourSunk(msgCondition, shipListPlayer1.get(1).getShiptype().getCoordinates());

            resetCoordinates(row, column, 5, shipListPlayer1.get(2).getShiptype().getCoordinates());
            msgCondition = shipThreeSunk(msgCondition, shipListPlayer1.get(2).getShiptype().getCoordinates());
